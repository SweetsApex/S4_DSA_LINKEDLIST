...
public class SingleLinkedList {
    ...
    public void insertInLinkedList(int nodeValue, int location){
        ...
        if(head == null) {
            ...
            node.next = null;
            tail = node;
        } else { // Inserting at a specific position
            Node tempNode = head;
            int index = 0;
    ...
    public void deleteNode(int location) {
        if (head == null) {
            ...
            return;
        } else if (location == 0) { // Deleting the head node
            head = head.next;
            size--;
            if (size == 0) { // If the list becomes empty
                tail = null;
            }
        } else if (location >= size) { // Deleting the tail node
            Node tempNode = head;
            ...
            tail = tempNode;
            size--;
        } else { // Deleting a node at a specific position
            Node tempNode = head;
